...
class SystemUserForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $frame_groups->style .= ';margin:0px;width:98%';
        ...
        $frontpage_name->setSize('calc(100% - 100px)');
        $program_id->setSize('100');
        $program_name->setSize('calc(90% - 200px)');
        ...
        $this->form->addFields( [$l3 = new TLabel(_t('Login'))], [$login],  [$l4 = new TLabel(_t('Email'))], [$email] );
        $this->form->addFields( [$l5 = new TLabel(_t('Unit'))], [$unit_id],  [$l6 = new TLabel(_t('Front page'))], [$frontpage_id, $frontpage_name] );
        $this->form->addFields( [$l7 = new TLabel(_t('Password'))], [$password],  [$l8 = new TLabel(_t('Password confirmation'))], [$repassword] );
        $id->placeholder = _t('Id');
        $name->placeholder = _t('Name');
        $login->placeholder = _t('Login');
        $email->placeholder = _t('Email');
        $frontpage_id->placeholder = _t('Front page');
        $frontpage_name->placeholder = _t('Front page');
