...
class SystemMessageForm extends TWindow
{
    ...
    function __construct()
    {
        ...
        // add the fields
        $this->form->addQuickField(_t('User'), $system_user_to_id,  '200');
        $this->form->addQuickField(_t('Subject'), $subject,  '100%' );
        $this->form->addQuickField(_t('Message'), $message,  '100%' );
        $message->setSize('100%', '100');
        
        $subject->addValidation(_t('User'), new TRequiredValidator);
