 ...
 */
class TerapyList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        parent::setActiveRecord('Terapy');   // defines the active record
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_Terapy');
        ...
        $this->form->setData( TSession::getValue('Terapy_filter_data') );
        
        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('Clear Filter'),  new TAction(array($this, 'onClear')), 'bs:ban-circle red');
        $this->form->addQuickAction(_t('New'),  new TAction(array('TerapyForm', 'onEdit')), 'bs:plus-sign green');
        ...
        $column_color = new TDataGridColumn('color', _t('Color'), 'center');
        $column_vacancies = new TDataGridColumn('vacancies', _t('Vacancies'), 'center');
        ...
        $this->datagrid->addColumn($column_vacancies);
        $this->datagrid->addColumn($column_weekdays);
        
        $column_color->setTransformer( function($value, $object, $row) {
            $label = '000000';
            $div = new TElement('span');
            $div->class="label";
            $div->style="background-color: " . $value . '; color: ' . $value;
            $div->add($label);
            return $div;
        });
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('TerapyForm', 'onEdit'));
    ...
    {        
        ...
        try
        {
            ...
            $repository = new TRepository('Terapy');
            $criteria = new TCriteria;
            ...
            if ($objects)
            {
                ...
                foreach ($objects as $object)
                {
                    ...
                    $designer->Cell(290,15,utf8_decode($object->name),0,0,'L',$fill);
                    $weekdaynames = array();
                    $repository = new TRepository('TerapyWeekday');
                    $criteria = new TCriteria;
                    $criteria->add(new TFilter('terapy_id','=',$object->id));
                    $terapyweekdays = $repository->load($criteria);
                    if ($terapyweekdays)
                    {
                        foreach ($terapyweekdays as $terapyweekday)
                        {
                            $weekday = new Weekday( $terapyweekday->weekday_id );
                            $weekdaynames[] = $weekday->name;
                        }
                    }
                    $designer->Cell(250,15,utf8_decode(implode(',',$weekdaynames)),0,0,'L',$fill);
                    $designer->Ln();
                    $fill = !$fill;
                }
                
                $designer->SetTextColor(0,126,196);
                $designer->Cell(290,15,'Total de Terapias',0,0,'R',$fill);
                $designer->Cell(250,15,$i,0,0,'L',$fill);
